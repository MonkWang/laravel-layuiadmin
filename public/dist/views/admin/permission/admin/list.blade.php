...
@section('content_script')
  ...
  layui.config({
    ...
    table.render({
      ...
      ,url: "{{route('admin.permission.member.list')}}" //模拟接口
      ,method: 'post'
    ...
    var active = {
      ...
      ,add: function(){
        layer.open({
          type: 2
          ,title: '添加管理员'
          ,content: '{{route('admin.permission.member.adminform')}}'
          ,area: ['420px', '420px']
          ,btn: ['确定', '取消']
          ,yes: function(index, layero){
            ...
            var iframeWindow = window['layui-layer-iframe'+ index]
            ,submitID = 'LAY-user-back-submit'
            ,submit = layero.find('iframe').contents().find('#'+ submitID);alert(33);
            // 监听提交
            iframeWindow.layui.form.on('submit(LAY-user-front-submit)', function(data){
              var field = data.field; //获取提交的字段
              //提交 Ajax 成功后，静态更新表格中的数据
              $.selfPost('{{route("admin.permission.member.adminform")}}', field)
              table.reload('LAY-user-front-submit'); //数据刷新
              layer.close(index); //关闭弹层
            });
            submit.trigger('click');
